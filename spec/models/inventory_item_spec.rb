...
RSpec.describe InventoryItem, type: :model do
  ...
  it 'invalid valid survivor_already_has_item' do
  ...
  it 'check_trade_issues' do 
  	...
  	expect((InventoryItem.check_trade_issues trade_params[:trade_items]).length ).to eq(0)
  end

    it 'invalid check_trade_issues' do 
  	s0 =  create(:survivor)
  	s1 =  create(:survivor)
  	i0 =  create(:item)
  	i1 =  create(:item)
  	...
  	trade_params = {
  	  	...
  	  	expect((InventoryItem.check_trade_issues trade_params[:trade_items]).length ).to be > 0
  end
